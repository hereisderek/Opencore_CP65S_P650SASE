...
if ARGV.count == 0
    ...
    file = ARGV.first
    puts color("Reading EDID from FILE" + file)
    edid_hex = File.read(file).gsub(/\s+/, "")
    puts "read edid:" + edid_hex
    bytes=edid_hex.scan(/../).map{|x|Integer("0x#{x}")}.flatten
end  
...
if edid[0] >> 7 != 0
  ...
  puts "HorizontalSyncPulseWidth: " + color(("%d" % bytes[64]),RED)
end
...
# FixHorizontalSyncPulseWidth
fixHorizontalSyncPulseWidth = 100 # 0x64
# puts "FixHorizontalSyncPulseWidth 0x" + Integer("0x#{bytes[63]}") + " => " + fixHorizontalSyncPulseWidth
bytes[64] = 100
